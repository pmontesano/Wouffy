...
import { formatDateTime, getWalkStatusLabel, getWalkStatusClass } from '../utils/api';
      ...
      <div className="min-h-screen bg-[#F9FAFB] flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl shadow-xl p-12 max-w-md text-center">
          ...
          <p className="text-gray-600 mb-6">
            Para empezar a recibir solicitudes, primero debés crear tu perfil de paseador.
          </p>
          <button
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {walks.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-lg p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">No tenés solicitudes aún</h3>
          ...
          <div className="space-y-4">
            {walks.map((walk, index) => (
              <motion.div
                ...
                className="bg-white rounded-3xl shadow-lg p-6 card-hover"
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex-1 mb-4 lg:mb-0">
                    <div className="flex items-center space-x-3 mb-3">
                      ...
                      <h3 className="text-xl font-bold text-[#1F2937]" style={{ fontFamily: 'Outfit' }}>
                        Paseo para {walk.pet_name}
                      </h3>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600">
                      <div className="flex items-center space-x-2">
                        ...
                        <span>{formatDateTime(walk.date_time_start)}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Clock size={16} className="text-[#FFCC99]" />
                        <span>{walk.duration_minutes} minutos</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <MapPin size={16} className="text-[#88D8B0]" />
                        <span>{walk.address_text}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <User size={16} className="text-[#FFCC99]" />
                        <span>Tamaño: {walk.pet_size}</span>
                      </div>
                    </div>

                    {walk.notes && (
                      <div className="mt-3 text-sm text-gray-600 bg-[#F9FAFB] p-3 rounded-xl">
                        <span className="font-semibold">Notas del dueño:</span> {walk.notes}
                      </div>
                    )}

                    {walk.pet_notes && (
                      <div className="mt-2 text-sm text-gray-600 bg-[#F9FAFB] p-3 rounded-xl">
                        <span className="font-semibold">Sobre la mascota:</span> {walk.pet_notes}
                      </div>
                    )}
                  </div>
